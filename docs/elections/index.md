# Élections professionnelles

Cette section regroupe les documents liés aux élections professionnelles à l'ICO (protocoles préélectoraux, vote électronique) ainsi que le dialogue social, distincts des accords collectifs permanents.

## Documents disponibles

*2 documents "plus en vigueur" ont été déplacés vers l'archive : voir [`../archive-accords-locaux/index.md`](../archive-accords-locaux/index.md) — exclus du travail de résumé aux salariés.*

| Document | Signature | Fichier |
|---|---|---|
| Vote électronique — cycle 2026 | 29/05/2026 | `ACCORD VOTE ELECTRONIQUE _ Elections professionnelles 2026.md` |
| Dialogue social 2026 | 13/03/2026 | `Accord dialogue social 2026.md` |

## En attente

- **PAPE (Protocole d'Accord Préélectoral)** : la version 2022 a été écartée du suivi (obsolète). Une version 2026 existera vraisemblablement pour les élections professionnelles de novembre 2026 — à ajouter ici dès réception (un PDF "Accord préparatoire PAPE 2022_Signé.pdf" est déjà dans ce dossier, pas encore converti).

## Nommage des fichiers

Le nom du fichier `.md` reprend exactement le nom du PDF signé correspondant (mêmes espaces, majuscules — mais accents retirés, pour éviter un problème d'encodage constaté sous Windows). Pour un document sans PDF, garder un nom clair de type `accord-[theme]-[annee].md`.
